...
public sealed class ExtractChangesRequestValidationTests
{
    ...
    [InlineData(-1)]
    [InlineData(-100)]
    public void Validate_Throws_WhenSinceServerGenIsNegative(long sinceServerGen) {
        var request = new ExtractChangesRequest {
            Layers = [0],
            ServerGens = new ExtractChangesServerGens {
                SinceServerGen = sinceServerGen
            }
        };
    ...
    [Theory]
    [InlineData(-1, 10, "MinServerGen")]
    [InlineData(1, -10, "MaxServerGen")]
    public void Validate_Throws_WhenServerGenRangeContainsNegativeValue(
        long minServerGen,
        long maxServerGen,
    ...
    [Fact]
    public void Validate_Throws_WhenLayerServerGenContainsNegativeServerGen() {
        var request = new ExtractChangesRequest {
            Layers = [0],
            LayerServerGens = [
                new ExtractChangesLayerServerGen(
                Id: 0,
                ServerGen: -1,
                MinServerGen: null)
            ]
        ...
        Assert.Contains("ServerGen", exception.Message, StringComparison.Ordinal);
        Assert.Contains("greater than or equal to zero", exception.Message, StringComparison.OrdinalIgnoreCase);
    }

    [Fact]
    public void Validate_Throws_WhenLayerServerGenContainsNegativeMinServerGen() {
        var request = new ExtractChangesRequest {
            Layers = [0],
            LayerServerGens = [
                new ExtractChangesLayerServerGen(
        ...
        Assert.Contains("MinServerGen", exception.Message, StringComparison.Ordinal);
        Assert.Contains("greater than or equal to zero", exception.Message, StringComparison.OrdinalIgnoreCase);
    }
