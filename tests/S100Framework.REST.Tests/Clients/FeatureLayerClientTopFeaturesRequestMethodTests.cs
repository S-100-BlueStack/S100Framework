...
public sealed class FeatureLayerClientTopFeaturesRequestMethodTests
{
    ...
    [Fact]
    public async Task QueryTopFeaturesAsync_PostFormPreservesObjectIdsAndSpatialFilter_WhenPosted() {
        var cancellationToken = TestContext.Current.CancellationToken;
        var topFeatureRequests = new List<HttpRequestMessage>();
        var formBodies = new List<string>();
        ...
        Assert.Equal("STATUS = 'Active'", form["where"]);
        Assert.Equal("10,20,30", form["objectIds"]);
        ...
        Assert.Equal("100", form["distance"]);
        Assert.Equal("esriSRUnit_Meter", form["units"]);
        Assert.True(form.ContainsKey("geometry"));

        Assert.Contains("\"groupByFields\":\"REGION,TYPE\"", form["topFilter"], StringComparison.Ordinal);
        Assert.Contains("\"orderByFields\":\"SCORE DESC,OBJECTID ASC\"", form["topFilter"], StringComparison.Ordinal);
        Assert.Contains("\"topCount\":3", form["topFilter"], StringComparison.Ordinal);
    }

    [Fact]
    public async Task QueryTopFeatureObjectIdsAsync_UsesPostForm_WhenAutoThresholdIsExceeded() {
        var cancellationToken = TestContext.Current.CancellationToken;
        var topFeatureRequests = new List<HttpRequestMessage>();
        var formBodies = new List<string>();
        ...
        Assert.StartsWith("NAME IN", form["where"], StringComparison.Ordinal);
        Assert.True(form.ContainsKey("topFilter"));
        Assert.False(form.ContainsKey("returnCountOnly"));
        Assert.False(form.ContainsKey("returnGeometry"));
        Assert.False(form.ContainsKey("outFields"));
    }

    [Fact]
    public async Task QueryTopFeatureObjectIdsAsync_UsesGet_WhenAutoThresholdIsNotExceeded() {
        var cancellationToken = TestContext.Current.CancellationToken;
        var topFeatureRequests = new List<HttpRequestMessage>();
    ...
    [Fact]
    public async Task QueryTopFeatureCountAsync_UsesPostForm_WhenAutoThresholdIsExceeded() {
        var cancellationToken = TestContext.Current.CancellationToken;
        var topFeatureRequests = new List<HttpRequestMessage>();
        var formBodies = new List<string>();
    ...
    [Fact]
    public async Task QueryTopFeatureCountAsync_UsesGet_WhenAutoThresholdIsNotExceeded() {
        var cancellationToken = TestContext.Current.CancellationToken;
        var topFeatureRequests = new List<HttpRequestMessage>();
